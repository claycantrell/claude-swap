...
- **Landing caps** (`autoswitch.landingMax5hPct`, `autoswitch.landingMax7dPct`): never switch *onto* an account whose own 5-hour or 7-day window is above the cap, whatever the strategy or trigger. Without them, an account at 97% on its 5-hour window is a legal target if it is under the threshold — and a running session that lands there hits its limit minutes later. With nothing under the caps, the engine waits for a reset rather than moving the problem to another account. Both default to 100 (off).
